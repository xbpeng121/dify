...
echo "  ✅ $HARBOR_URL/$HARBOR_PROJECT/sibat-dify-web:0.15.6"
echo "  ✅ $HARBOR_URL/$HARBOR_PROJECT/sibat-dify-api:0.15.6"
echo "  ✅ $HARBOR_URL/$HARBOR_PROJECT/langgenius/dify-sandbox:0.2.11"
echo ""
...
if [ -f "api-deployment.yaml" ]; then
    sed -i "s|image:.*dify-api.*|image: $HARBOR_URL/$HARBOR_PROJECT/sibat-dify-api:0.15.6|g" api-deployment.yaml
    log_info "✅ 已更新 api-deployment.yaml"
fi

if [ -f "worker-deployment.yaml" ]; then
    sed -i "s|image:.*dify-api.*|image: $HARBOR_URL/$HARBOR_PROJECT/sibat-dify-api:0.15.6|g" worker-deployment.yaml
    log_info "✅ 已更新 worker-deployment.yaml"
fi

if [ -f "web-deployment.yaml" ]; then
    sed -i "s|image:.*dify-web.*|image: $HARBOR_URL/$HARBOR_PROJECT/sibat-dify-web:0.15.6|g" web-deployment.yaml
    log_info "✅ 已更新 web-deployment.yaml"
fi

if [ -f "sandbox-deployment.yaml" ]; then
    sed -i "s|image:.*dify-sandbox.*|image: $HARBOR_URL/$HARBOR_PROJECT/langgenius/dify-sandbox:0.2.11|g" sandbox-deployment.yaml

  ...
  <Col>
    ...
    ### Request Body
      ...
      - `files` (array[object]) Optional
          ...
          - `transfer_method` (string) 传递方式，`remote_url` 图片地址 / `local_file` 上传文件 / `base64` 转为base64在json中传输
          - `url` (string) 图片地址（仅当传递方式为 `remote_url` 时）
          - `upload_file_id` (string)  上传文件 ID（仅当传递方式为 `local_file` 时）
          - `base64` (string) 文件的base64 url（仅当传递方式为`base64`时）
      - `response_mode` (string) Required
        返回响应模式，支持：
        - `streaming` 流式模式（推荐）。基于 SSE（**[Server-Sent Events](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events)**）实现类似打字机输出方式的流式返回。
        - `blocking` 阻塞模式，等待执行完毕后返回结果。（请求若流程较长可能会被中断）。
...
<Row>
  <Col>
    ...
    <Properties>
      <Property name='file' type='file(Required)' key='file'>
        要上传的文件。
      </Property>
      <Property name='user' type='string(Required)' key='user'>
          用户标识，用于定义终端用户的身份，必须和发送消息接口传入 user 保持一致。
      </Property>
      <Property name='is_temporary' type='bool(Optional)' key='is_temporary'>
          临时文件标识，默认值true，文件将会在1天后自动删除，你可以在.env中通过设置CLEAN_UPLOAD_TEMPORARY_FILE_DAY_SETTING参数调整删除时间。
      </Property>
    </Properties>
...
<Row>
  <Col>
    用于第一次使用工作流时，获取结束节点的输出参数名称、类型及默认值等。

    ### Response
    - `workflow_output_form` (array[object]) 工作流结束节点的输出参数
      - `end_node_title` (string) 结束节点名称
      - `variables` (array[object]) 该结束节点的输出参数列表
        - `variable` (string) 输出参数名称
        - `value_selector` (array[string]) 参数选择器（用于定位参数来源） 
        - `value_type` (string) 参数类型, 枚举值来源： ["number","string","object","file","array[string]","array[number]","array[object]","array[file]","none"]

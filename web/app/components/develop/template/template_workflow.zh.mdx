      ...
      - `inputs` (object) Required
        ...
        文件列表类型变量适用于传入文件结合文本理解并回答问题，仅当模型支持该类型文件解析能力时可用。如果该变量是文件列表类型，该变量对应的值应是列表格式，其中每个元素应包含以下内容：
          ...
          - `transfer_method` (string) 传递方式：
            - `remote_url` 图片地址
            - `local_file` 上传文件
            - `base64` Base64 编码文件（支持纯 base64 或 Data URL 格式）
          - `url` (string) 图片地址（仅当传递方式为 `remote_url` 时）
          - `upload_file_id` (string) 上传文件 ID（仅当传递方式为 `local_file` 时）
          - `base64_data` (string) Base64 编码的文件数据（仅当传递方式为 `base64` 时）
            支持两种格式：
...
<Row>
  ...
  - `mode` (string) 应用模式
  - `app_mode` (string) 应用模式（与 mode 相同）
  - `app_id` (string) 应用 ID
...
<Row>
  <Col>
  用于获取工作流的输出结构定义，返回所有结束节点的输出变量信息。

  此接口可在执行工作流前调用，用于了解工作流会返回哪些输出参数及其类型。

  ### Response
  - `workflow_output_form` (array[object]) 工作流输出结构列表
    - `end_node_title` (string) 结束节点标题
    - `variables` (array[object]) 输出变量列表
      - `variable` (string) 变量名称
      - `value_type` (string) 变量类型，可能的值：
        - `string` 字符串
        - `number` 数字
        - `object` 对象
        - `array[string]` 字符串数组
        ...
        - `array[object]` 对象数组
        - `array[file]` 文件数组
      - `value_selector` (array[string]) 变量选择器路径
  </Col>
  <Col>
    <CodeGroup
      title="Request"

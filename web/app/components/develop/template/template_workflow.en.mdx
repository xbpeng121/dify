...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="GET" label="/workflow/output" targetCode={` curl -X GET '${props.appDetail.api_base_url}/workflow/output' \\\n-H 'Authorization: Bearer {api_key}'`}>

    ```bash {{ title: 'cURL' }}
    curl -X GET '${props.appDetail.api_base_url}/workflow/output' \
    --header 'Authorization: Bearer {api_key}'
    ```

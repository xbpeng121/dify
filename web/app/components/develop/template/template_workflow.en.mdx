      ...
      - `inputs` (object) Required
        ...
        If the variable is of File Array type, the corresponding value should be a list whose elements contain following attributions:
          ...
          - `transfer_method` (string) Transfer method:
            ...
            - `base64`: Base64 encoded file (supports both pure base64 and Data URL format).
          ...
          - `base64_data` (string) Base64 encoded file data (Only when transfer method is `base64`)
            Supports two formats:
            - Pure base64: `iVBORw0KGgoAAAANSUhEUgA...`
            - Data URL: `data:image/png;base64,iVBORw0KGgoAAAANSUhEUgA...`
            File size limit: 15MB (after decoding)
...
--data-raw '{
    ...
    <CodeGroup title="Example: base64 file as an input variable">
      ```json {{ title: 'Base64 file example' }}
      {
        "inputs": {
          "image": [
            {
    ...
    <CodeGroup title="Example: Data URL format base64 file">
      ```json {{ title: 'Data URL format example' }}
      {
        "inputs": {
          "image": [
            {
    ...
    ### Request Body
      - `inputs` (object) Required
        ...
        If the variable is of File Array type, the corresponding value should be a list whose elements contain following attributions:
          ...
          - `transfer_method` (string) Transfer method, `remote_url` for image URL / `local_file` for file upload / `base64` for Base64 encoded file
          - `url` (string) Image URL (when the transfer method is `remote_url`)
          - `upload_file_id` (string) Uploaded file ID, which must be obtained by uploading through the File Upload API in advance (when the transfer method is `local_file`)
          - `base64_data` (string) Base64 encoded file data (when the transfer method is `base64`)
            Supports two formats:
            - Pure base64: `iVBORw0KGgoAAAANSUhEUgA...`
            - Data URL: `data:image/png;base64,iVBORw0KGgoAAAANSUhEUgA...`
            File size limit: 15MB (after decoding)
...
<Row>
  ...
  - `app_mode` (string) application mode (same as mode)
...
<Heading
  ...
  method='GET'
  title='Get Workflow Output Schema'
  name='#workflow-output'
/>
<Row>
  <Col>
  Used to get the workflow output schema definition, returning output variable information from all end nodes.

  This endpoint can be called before executing the workflow to understand what output parameters and their types the workflow will return.

  ### Response
  - `workflow_output_form` (array[object]) Workflow output schema list
    - `end_node_title` (string) End node title
    - `variables` (array[object]) Output variable list
      - `variable` (string) Variable name
      - `value_type` (string) Variable type, possible values:
        - `string` String
        - `number` Number
        - `object` Object
      ...
      - `value_selector` (array[string]) Variable selector path
  </Col>
  <Col>
    <CodeGroup
      title="Request"
      tag="GET"
      label="/workflows/output"
      targetCode={`curl -X GET '${props.appDetail.api_base_url}/workflows/output' \\
-H 'Authorization: Bearer {api_key}'`}
    />
    <CodeGroup title="Response">
    ```json {{ title: 'Response' }}
          ...
          "variables": [
            {
              "variable": "result",
              "value_type": "string",
              "value_selector": ["1234567890", "text"]
            },
            {
              "variable": "summary",
              "value_type": "string",
              "value_selector": ["0987654321", "output"]
            }
          ]
        }
      ]

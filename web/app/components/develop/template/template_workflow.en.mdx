    ...
    ### Request Body
      ...
      - `files` (array[object]) Optional
        File list, suitable for inputting files combined with text understanding and answering questions, available only when the model supports file parsing and understanding capability.
          ...
          - `transfer_method` (string) Transfer method, `remote_url` for image URL / `local_file` for file upload / `base64` for base64 string in json
          - `url` (string) Image URL (when the transfer method is `remote_url`)
          - `upload_file_id` (string) Uploaded file ID, which must be obtained by uploading through the File Upload API in advance (when the transfer method is `local_file`)
          - `base64` (string) base64 encoded string, (when the transfer method is `base64`)
...
<Row>
  ...
  - `is_temporary` (bool) Optional（default true）
    file will be deleted by system automatically, The default expired_time is 1 day.
    To chanage expired_time by set CLEAN_UPLOAD_TEMPORARY_FILE_DAY_SETTING in .env file.
...
<Heading
  ...
  method='GET'
  title='Get workflow output Information'
  name='#/workflows/output'
/>
<Row>
  <Col>
    Used at the first time to obtain workflow output information such as output parameter names, types, and default values.

    ### Response
    - `workflow_output_form` (array[object]) workflow output from end_nodes
      - `end_node_title` (string) The title of the end node
      - `variables` (array[object]) variables outputs in this end node
        - `variable` (string) the name of the variable
        - `value_selector` (array[string]) variables selector which is used to locate variable 
        - `value_type` (string) the type of the variable, enums : ["number","string","object","file","array[string]","array[number]","array[object]","array[file]","none"]

  </Col>
  <Col sticky>

    <CodeGroup title="Request" tag="GET" label="/workflows/output" targetCode={` curl -X GET '${props.appDetail.api_base_url}/workflows/output' \\\n-H 'Authorization: Bearer {api_key}'`}>

    ```bash {{ title: 'cURL' }}
    curl -X GET '${props.appDetail.api_base_url}/workflows/output' \
    --header 'Authorization: Bearer {api_key}'
    ```

      ...
      - `inputs` (object) 必須
        ...
        変数がファイルリストの場合、リストの各要素は以下の属性を持つ必要があります。
          ...
          - `transfer_method` (string) 転送方法:
            ...
            - `base64`: Base64エンコードされたファイル（純粋なbase64またはData URL形式をサポート）。
          - `url` ファイルのURL。（転送方法が `remote_url` の場合のみ）。
          - `upload_file_id` アップロードされたファイルID。（転送方法が `local_file` の場合のみ）。
          - `base64_data` (string) Base64エンコードされたファイルデータ（転送方法が `base64` の場合のみ）
            2つの形式をサポート：
            - 純粋なbase64：`iVBORw0KGgoAAAANSUhEUgA...`
            - Data URL：`data:image/png;base64,iVBORw0KGgoAAAANSUhEUgA...`
            ファイルサイズ制限：15MB（デコード後）
...
--data-raw '{
    ...
    <CodeGroup title="Base64ファイルの例">
      ```json {{ title: 'Base64ファイルの例' }}
      {
        "inputs": {
          "image": [
            {
    ...
    <CodeGroup title="Data URL形式のBase64ファイルの例">
      ```json {{ title: 'Data URL形式の例' }}
      {
        "inputs": {
          "image": [
            {
    ...
    ### リクエストボディ
      - `inputs` (object) 必須
        ...
        ファイルリスト型変数は、ファイルをテキスト理解と組み合わせて質問に答えるために入力するのに適しており、モデルがファイル解析機能をサポートしている場合のみ使用できます。変数がファイルリスト型の場合、その変数に対応する値はリスト形式である必要があり、各要素には以下の内容が含まれます：
          ...
          - `transfer_method` (string) 転送方法、`remote_url` 画像URL / `local_file` ファイルアップロード / `base64` Base64エンコードされたファイル
...
<Row>
  ...
  - `app_mode` (string) アプリケーションのモード（modeと同じ）
...
<Heading
  ...
  method='GET'
  title='ワークフロー出力スキーマを取得'
  name='#workflow-output'
/>
<Row>
  <Col>
  ワークフローの出力スキーマ定義を取得し、すべての終了ノードからの出力変数情報を返します。

  このエンドポイントは、ワークフローを実行する前に呼び出して、ワークフローが返す出力パラメータとその型を理解することができます。

  ### Response
  - `workflow_output_form` (array[object]) ワークフロー出力スキーマリスト
    - `end_node_title` (string) 終了ノードのタイトル
    - `variables` (array[object]) 出力変数リスト
      - `variable` (string) 変数名
      - `value_type` (string) 変数の型、可能な値：
        - `string` 文字列
        - `number` 数値
        - `object` オブジェクト
        - `array[string]` 文字列配列
        ...
        - `array[file]` ファイル配列
      - `value_selector` (array[string]) 変数セレクタパス
  </Col>
  <Col>
    <CodeGroup
      title="Request"
